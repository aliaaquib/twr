  ...
  return (
    <section className="rounded-xl border border-stone-200 bg-stone-50 p-3.5 dark:border-stone-800 dark:bg-[linear-gradient(180deg,rgba(34,30,27,0.98),rgba(24,21,19,0.98))] dark:ring-1 dark:ring-inset dark:ring-stone-800/80 sm:p-4">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold tracking-tight text-stone-900 dark:text-stone-100 sm:text-xl">What&apos;s hot in AI</h2>
      ...
      <div className="mt-5 space-y-3.5 sm:mt-6 sm:space-y-4">
        ...
        {visibleItems.map((item) => (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="block py-0.5 transition hover:opacity-90 sm:py-1"
          >
            <div className="flex items-start gap-3">
              <span className="mt-0.5 text-lg">{SOURCE_ICONS[item.source] || "🔥"}</span>
              <div className="min-w-0 flex-1">
                <h3 className="text-sm font-medium leading-5 text-stone-900 dark:text-stone-100 sm:leading-6">{item.title}</h3>
                <div className="mt-2 text-xs text-stone-500 dark:text-stone-400 sm:mt-3">
